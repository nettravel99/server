...
function verifyUser(req, res, next) {
  ...
  db
    ...
    .then(function (data) {
      ...
      // data.salt and data.password are the values in the datatbase. reqPswd is from
      // the user logging in.
      result = pswdMethods.validate(data.salt, data.password, reqPswd, data.email);
      console.log("result:", result);
      //var result = true;
      if (result) {
        res
          .status(200)
          .json({status: "success", data: result, message: "Retrieved User Data"});
        /* jshint ignore:end */
      } else {
        res
          .status(400)
...
async function confirm(req, res, next) {
  ...
  //console.log("email", email); console.log("password", reqPswd);
  // TODO? - email should be transformed to lower case and kept in lower case.
  // db.one("select * from users where email = $1", email)
  let edata;
  try {
  ...
  let x = edata[0]['user_token_exists2'];
...
// var dbmethods = require("../db/db"); // var path = require('path');
//
// db = dbmethods.getDB();
//
// function verifyUser(req, res, next) {   console.log("verify called",
// req.params.id);   var userID = parseInt(req.params.id);   db.one("select *
// from \"users\" where id = '$1'", userID)     .then(function(data) {
// res.status(200).json({         status: "success",         data: data,
// message: "Retrieved User Data"       });       /* jshint ignore:end */     })
